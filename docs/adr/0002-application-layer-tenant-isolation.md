...
Every org-owned row (projects today, issues later) must be scoped by `organizationId`. The access JWT only identifies the user; org context comes from the `x-org-id` header on tenant routes.

The first project endpoints did the right things manually: read the header, check membership, pass `organizationId` into module queries. That already broke down - `update` scoped by `id` only while `findById` included org. Copy-pasting membership checks into every `api/` service would get worse as we add issues, comments, webhooks, and jobs.
...
Auth routes and `GET /organizations` skip the org guard - they have no active tenant.

### OrgMemberGuard

Runs on controllers like `ProjectController`. Parses `x-org-id`, calls `OrgMemberService.findByOrgAndUser`, throws `403` if not a member, then sets CLS. Membership logic lives here once; `api/` services do not repeat it.
...
Jobs and Stripe webhooks will set CLS the same way before calling module code (org from job payload or Stripe customer lookup). No separate scoping path.

### Prisma extension

`createTenantScopedClient()` wraps the base client. Models on the allowlist (currently `Project`) get `organizationId` merged from CLS into `where` / `data`. Missing CLS on a tenant operation throws - that is a bug, not a silent cross-tenant read.
...
`test/tenant-isolation.e2e-spec.ts` - org B cannot GET or PATCH org A's project (404). Same file checks module `findById` / `update` with org B in CLS but no `organizationId` in the query args; extension returns null.
